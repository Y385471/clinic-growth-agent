...
// The attended line is written at run time, before the receptionist ticks. Once attendance is complete,
// speak from the real numbers instead. Returns null when the stored line is still right.
export function freshAttendedLine(f, summary) {
  const stale = f.attendanceComplete && summary?.attendance_complete_at_run !== true && f.counts.booked > 0;
  if (!stale) return null;
  return {
    problem: `حضر ${f.counts.attended} من ${f.counts.booked} حجزوا (${Math.round((f.counts.attended / f.counts.booked) * 100)}%).`,
    fix: f.counts.attended < f.counts.booked
      ? 'ابعت رسالة تذكير قبل الميعاد بيوم، واتصل باللي ما حضروش تحجزلهم ميعاد جديد.'
      : 'كل اللي حجزوا حضروا: كمّل على نفس رسالة التأكيد.',
  };
}
