...
import { recheck, propose, forgetDay } from './learn.js';
...
export async function runDay(day, { onSend, log = () => {}, simulateSourceError = false } = {}) {
  if (simulateSourceError) throw new Error('simulated: the page could not be reached');
  ...
  // 5. Learn: re-check yesterday's hypotheses against today's numbers (verified by code).
  await forgetDay(day);
  const dayData = { day, ads: data.ads, conversations, bookings, bookedIds };
  const rechecked = await recheck(dayData);
  log(`rechecked ${rechecked.length} hypotheses: ${rechecked.map(h => h.status).join(', ') || '—'}`);
  ...
  const stats = {
    ...
    learned_from_previous_days: rechecked.map(h => ({ hypothesis: h.statement, status: h.status, today: h.evidence.at(-1)?.text })),
  };
  ...
  // New hypotheses: at most one per stage that loses 30% or more of the people coming in.
  const leaking = funnel.transitions.filter(t => t.lostShare >= 0.3).map(t => t.to);
  let proposed = [];
  try { proposed = await propose(dayData, leaking, stats, { onSend }); }
  catch (e) { log(`no new hypotheses: ${e.message}`); }
  log(`proposed ${proposed.length}: ${proposed.map(h => `${h.metric} ${h.direction} ${h.threshold}`).join('; ')}`);

  // 6. Data continuity: if the previous day's attendance was never ticked, alert the owner.
