...
import { insert, upsert, remove } from './db.js';
...
export async function runDay(day, { onSend, log = () => {} } = {}) {
  ...
  const scrubbed = data.conversations.map((c, i) => {
    const alias = `مريض ${day}-${String(i + 1).padStart(2, '0')}`;
  ...
  const run = {
    day, mode: mode(), status: analyseError ? 'partial' : 'ok', alert: null,
    error: analyseError, summary: { stages: summary, leak: funnel.leak, model: model || summaryModel },
